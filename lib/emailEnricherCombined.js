...
export async function enrichLeadEmail({ lead, supabase, log = null, persist = true }) {
  ...
  // The hunterEnrichment wrapper renames API fields:
  //   value → email · first_name → firstName · last_name → lastName
  if (process.env.HUNTER_API_KEY) {
    try {
      ...
      if (personalHit?.email) {
        result = {
          email:            personalHit.email.toLowerCase(),
          source:           'hunter',
          confidence:       personalHit.confidence / 100,
          persona_name:     [personalHit.firstName, personalHit.lastName].filter(Boolean).join(' ') || null,
          persona_position: personalHit.position || null,
          all_found:        personalSearch.emails.map((e) => e.email).filter(Boolean),
        };
      }
      ...
      if (!result) {
        ...
        if (genericHit?.email) {
          result = {
            email:            genericHit.email.toLowerCase(),
            source:           'hunter',
            confidence:       genericHit.confidence / 100,
            persona_name:     null,
            persona_position: null,
            all_found:        genericSearch.emails.map((e) => e.email).filter(Boolean),
          };
        }
      }
    } catch (err) {
