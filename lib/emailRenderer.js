...
export function renderMagnetEmail(magnetData, lead, options = {}) {
  ...
  if (magnetData.magnet_type === 'website_screenshot') {
    ...
    if (angelaBody) {
      bodyParagraphs = sanitizeAgentBody(angelaBody)
        .split('\n')
    ...
    // Clean up LLM's body
    let rawBody = sanitizeAgentBody(angela_email_body || '');
    ...
    if (currentAssetUrl) {
      if (isLocalPath(currentAssetUrl)) {
        let absolutePath;
        try {
          absolutePath = path.resolve(currentAssetUrl);
        } catch (e) {
          absolutePath = null;
        }
        if (absolutePath && fs.existsSync(absolutePath)) {
          ...
          console.warn(`  ⚠️ [emailRenderer] Asset local no accesible, skip visual: ${currentAssetUrl}`);
        }
      } else {
...
// Strip HTML tags, style blocks, markdown, and unresolved placeholders
// from agent-generated body text. Defense in depth against LLMs that
// leak HTML despite prompt instructions.
function sanitizeAgentBody(str) {
  if (!str) return '';
  let s = String(str);
  ...
  s = s.replace(/\[[^\]]*?(enlace|link|url|propuesta|mockup)[^\]]*?\]/gi, '');
  s = s.replace(/\{\{[^}]+\}\}/g, '').replace(/\{[a-z_][a-z0-9_]*\}/gi, '');
