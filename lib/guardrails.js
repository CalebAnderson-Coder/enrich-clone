...
/**
 * Bounce rate over the last 24h for a brand. FALLBACK implementation
 * that reads from `campaign_enriched_data.outreach_status` so the
 * circuit breaker is no longer a silent no-op. Will migrate to the
 * dedicated `outreach_events` table in Sprint 5.
 *
 * Rate = bounced / (sent + bounced). When no rows match the window
 * we return 0 (breaker inactive, no data to evaluate).
 */
export async function getBounceRateLast24h(brandId, { client } = {}) {
  ...
  try {
    ...
    if (rate > threshold) {
      log.warn('bounce rate breach', { brandId, rate, bounced, total, threshold });
    }
    return rate;
  } catch {
    return 0;
