...
export class AgentMessenger {
  ...
  /**
   * Reclaim stale-claimed messages and mark crashed sibling processes.
   *
   * Call once at worker boot (after `register()`) to clean up after a
   * previous crash of this same agent type. Two operations are performed,
   * both scoped to `this.brandId` and `this.agentName`:
   *
   *   1. **Mark crashed processes:** Any `agent_processes` row for this
   *      agent that is still `status='running'` but whose
   *      `last_heartbeat_at` is older than `staleAfterMs` milliseconds is
   *      flipped to `status='crashed'`. The currently-running process
   *      (`this.processId`) is excluded so it never marks itself crashed.
   *
   *   2. **Reclaim their messages:** Any `agent_messages` row addressed to
   *      this agent that is still `status='claimed'` and whose `claimed_by`
   *      belongs to one of the just-crashed processes is reset to
   *      `status='pending'` so the live worker can pick it up in the next
   *      `recv()` call. The `error` field is set to
   *      `'reclaimed_from_crashed_worker'` for observability.
   *      A 2-pass approach is used (SELECT crashed process_ids, then
   *      UPDATE messages) because the Supabase JS client does not expose
   *      JOIN-style UPDATE predicates directly.
   *
   * @param {object} [opts]
   * @param {number} [opts.staleAfterMs=120000]  Milliseconds after which a
   *   non-heartbeating running process is considered crashed (default 2 min).
   * @returns {Promise<{ crashedProcesses: number, reclaimedMessages: number }>}
   */
  async reclaimOrphans({ staleAfterMs = 120_000 } = {}) {
    const staleThreshold = new Date(Date.now() - staleAfterMs).toISOString();

    // ── Pass 1: mark stale running processes as crashed ─────────
    const { data: crashedRows, error: crashErr } = await this._db
      .from('agent_processes')
      ...
      .neq('process_id', this.processId)
      .select('process_id');

    if (crashErr) {
      throw new AgentMessengerError(
        `reclaimOrphans: failed to mark crashed processes: ${crashErr.message}`,
        { crashErr }
      );
    }

    const crashedIds = (crashedRows ?? []).map((r) => r.process_id);
    const crashedProcesses = crashedIds.length;

    if (crashedProcesses === 0) {
      return { crashedProcesses: 0, reclaimedMessages: 0 };
    }

    // ── Pass 2: reclaim messages claimed by those crashed processes ─
    const { data: reclaimedRows, error: reclaimErr } = await this._db
      .from('agent_messages')
      .update({
        status:     'pending',
        claimed_by: null,
        claimed_at: null,
        error:      'reclaimed_from_crashed_worker',
      })
      .eq('to_agent',  this.agentName)
      .eq('brand_id',  this.brandId)
      .eq('status',    'claimed')
      .in('claimed_by', crashedIds)
      .select('id');

    if (reclaimErr) {
      throw new AgentMessengerError(
        `reclaimOrphans: failed to reclaim messages: ${reclaimErr.message}`,
        { reclaimErr }
      );
    }

    const reclaimedMessages = (reclaimedRows ?? []).length;
    return { crashedProcesses, reclaimedMessages };
  }
