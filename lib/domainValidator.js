...
export async function isDomainReachable(urlOrDomain, timeoutMs = 5000) {
  ...
  try {
    ...
    clearTimeout(timer);
    // 2xx/3xx = alive. 4xx often means server rejects HEAD (405/403) —
    // retry with GET before declaring dead. 5xx falls through to GET too.
    if (response.ok || (response.status >= 300 && response.status < 400)) {
      return { reachable: true, statusCode: response.status };
    }
    // fall through to GET retry
  } catch (err) {
    // network/timeout — also fall through to GET retry
  }

  {
    try {
      ...
      return { reachable: response2.ok || (response2.status >= 300 && response2.status < 400), statusCode: response2.status };
    } catch (err2) {
