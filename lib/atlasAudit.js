...
export async function runAtlasAudit({
  ...
  // Dedupe — pick the row with newest started_at per agent (auditor finding F5):
  // a brand-new crashed process beats a stale "alive" zombie row.
  ...
  for (const row of procRows) {
    ...
    if (!prev) {
      ...
      const prevStart = prev.started_at ? new Date(prev.started_at).getTime() : 0;
      const curStart  = row.started_at  ? new Date(row.started_at).getTime()  : 0;
      if (curStart > prevStart) latestByAgent.set(key, row);
    }
  }
