  ...
  async _runWithClient(agentName, agent, userMessage, context, client, model, providerName) {
    ...
    while (iteration < maxIterations) {
      ...
      if (totalToolCalls >= maxToolCalls) {
        console.log(`  ⚡ [${agentName}] Max tool calls (${maxToolCalls}) reached — forcing final response`);
        messages.push({
          role: 'user',
      ...
      for (const toolCall of message.tool_calls) {
        ...
        console.log(`  🔧 [${agentName}] calling ${fnName}(${Object.keys(args).join(', ')}) [${totalToolCalls}/${maxToolCalls}]`);
