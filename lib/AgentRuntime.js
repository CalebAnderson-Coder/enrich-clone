...
export class AgentRuntime {
  ...
  async run(agentName, userMessage, context = {}) {
    ...
    // BK-028: _forceProvider lets a caller skip primary and go straight
    // to the fallback client (e.g. Verifier path with strict-JSON output
    // — see verifierGate.js TODO). Falls back to normal flow if the
    // requested provider isn't available, its breaker is OPEN, or the
    // value isn't the fallback provider's name.
    const fallbackProviderName = this.fallbackCB?.name || 'gemini';
    const forced = String(context._forceProvider || '').toLowerCase();
    const forceFallback =
      forced && forced === fallbackProviderName.toLowerCase()
      && !!this.geminiClient
      && this.fallbackCB?.state !== 'OPEN';

    // Try primary model. Skip entirely if its breaker is OPEN — go straight to fallback.
    // Also skip if the caller explicitly forced the fallback provider.
    const primaryProviderName = this.primaryCB?.name || 'NVIDIA';
    let result;
    if (forceFallback) {
      rootLogger.info(`[AgentRuntime] _forceProvider=${forced} — skipping primary`, { agent: agentName });
      result = await this._runWithClient(
        agentName, agent, userMessage,
        { ...runContext, _isGeminiFallback: true },
        this.geminiClient, this.geminiModel, 'Gemini'
      );
      result.fallbackUsed = true;
      result.forcedProvider = forced;
      if (!context._isGeminiFallback) {
        recordAgentEvent({
          trace_id: traceId,
          brand_id: brandId,
