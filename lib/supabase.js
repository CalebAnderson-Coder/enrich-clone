...
export async function getActiveBrands({ onlyAutonomous = false, client } = {}) {
  const db = client || supabase;
  if (!db) return [{ id: 'demo-brand', name: 'Demo Brand' }];

  if (onlyAutonomous) {
    // Burn protection: only brands that have an explicit row in brand_quota
    // with warmup_stage >= 1 are considered opted into autonomy. Missing row
    // → owner never configured quota → brand stays out of the autonomous loop.
    const { data: quotaRows, error: quotaErr } = await db
      .from('brand_quota')
      .select('brand_id, warmup_stage')
      .gte('warmup_stage', 1);
    if (quotaErr) return [];
    const ids = (quotaRows || []).map(r => r.brand_id).filter(Boolean);
