...
export async function verifyAndRewrite(draft, leadContext, runtime, { maxRetries = 2 } = {}) {
  const { businessName = '', industry = '', metro = '', tier = '', brandId } = leadContext || {};
  const verifier_history = [];
  let currentDraft = draft;
  // BK-004: propagate brandId so agent_events inserts satisfy NOT NULL.
  const runContext = { currentAgent: 'Verifier', ...(brandId ? { brandId } : {}) };
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      result = await runtime.run('Verifier', evalPrompt, runContext);
    } catch (err) {
    ...
    let rewriteResult;
    try {
      rewriteResult = await runtime.run('Angela', rewritePrompt, { ...runContext, currentAgent: 'Angela' });
    } catch (err) {
