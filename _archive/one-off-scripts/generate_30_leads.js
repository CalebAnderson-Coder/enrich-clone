...
(async () => {
    console.log(`\n🚀 Iniciando Extracción en Masa para generar 30 HOT LEADS de ${NICHE} en ${CITY}...`);
    ...
    const prompt = `You are a Lead Generation API. Your task is to generate EXACTLY 30 highly realistic, unique, Hispanic/Latino-owned service business leads for the niche: "${NICHE}" in "${CITY}".
Return a JSON array of 30 objects. Make the data diverse and extremely realistic for a CRM.
...
ONLY return the JSON array of exactly 30 leads.`;

    console.log("⏳ Generando data de 30 leads...");
    ...
    for (const chunk of chunks) {
        ...
        const promises = chunk.map(async (leadData) => {
            ...
            try {
                ...
                console.log(`[Lead ${index}/30] 📝 ${leadData.business_name} guardado. Ejecutando Escuadrón de Enriquecimiento...`);
...
4. Devuélveme al Manager toda la info consolidada incluyendo el análisis, estrategia y copy.`;
                ...
                console.log(`[Lead ${index}/30] ✅ ¡Enriquecimiento y Ataque listos para ${leadData.business_name}!`);

            } catch (err) {
                console.log(`[Lead ${index}/30] ⚠️ Error con ${leadData.business_name}:`, err.message);
            }
        });
    ...
    console.log(`\n🎉🔥 ¡MISION CUMPLIDA! Se inyectaron 30 leads HOT y totalmente enriquecidos en tu base de datos.`);
    process.exit(0);
})();
